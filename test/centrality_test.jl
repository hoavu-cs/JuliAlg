...
@testset "Betweenness Centrality Tests" begin
    ...
    @testset "Empty and trivial graphs" begin
        ...
        @testset "Two vertices, one edge" begin
            g = SimpleGraph(2)
            add_edge!(g, 1, 2)
            bc = bw_centrality(g, normalized=false)
    ...
    @testset "Weighted directed graphs" begin
        @testset "Weighted path where shortcut is longer" begin
            ...
            bc = bw_centrality(g, weights=weights, normalized=false)
            @test bc ≈ [0.0, 2.0, 2.0, 0.0] atol=1e-10
            @test bc[2] > 0
            @test bc[3] > 0
        end
        ...
        @testset "Equal weights match unweighted" begin
            ...
            bc_w = bw_centrality(g, weights=weights, normalized=false)
            bc_u = bw_centrality(g, normalized=false)
    ...
    @testset "Normalization" begin
        @testset "Normalized undirected" begin
            ...
            bc_norm = bw_centrality(g, normalized=true)
    ...
    @testset "Convenience method" begin
        g = SimpleDiGraph(4)
        add_edge!(g, 1, 2)
        add_edge!(g, 2, 3)
        add_edge!(g, 3, 4)
        add_edge!(g, 1, 4)
        weights = Dict((1, 2) => 2.0, (2, 3) => 1.0, (3, 4) => 3.0, (1, 4) => 5.0)
        bc1 = bw_centrality(g, weights=weights)
        bc2 = bw_centrality(g, weights)
        expected = [0.0, 1/6, 1/6, 0.0]
        @test bc1 ≈ expected atol=1e-10
        @test bc2 ≈ expected atol=1e-10
    end
